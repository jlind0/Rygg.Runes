...
from django.contrib.auth import login as oauthlogin
from django.contrib.auth import logout as oauthlogout
...
def image_upload(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            detector = RuneDetection(
                str(Path(__file__).parent / "rune_ocr_v2__yolov7_512_HQ2.onnx")
            )
            annotations, img_annotated = detector.run(
                image=imgincomming, return_image=True, score_thresh=0.1
            )
            upload_blob(
                "rune-images",
                fileNameBase,
...
def post_image(request):
    if request.method == "POST":
        ...
        annotations, img_annotated = detector.run(
            image=image, return_image=True, score_thresh=0.1
        )
        upload_blob(
            "rune-images",
            fileNameBase,
